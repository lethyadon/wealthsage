...
export default function Dashboard() {
  ...
  const processTransactions = (data) => {
    ...
    setTotalSpend(spend);
    const rec = Object.entries(count).filter(([d,t])=>t>1||subscriptionKeywords.some(k=>d.includes(k))).map(([d])=>d);
    setRecurring(rec);
    ...
    if(rec.filter(r=>/tinder/.test(r)).length>2) recs.push(`• Tinder: free version or pause can save £15–£20.`);
    if(rec.filter(r=>/deliveroo|ubereats/.test(r)).length>2) recs.push('• Food Delivery: cook at home 3x/week = save ~£70/mo.');
    if(cats['Transport']) recs.push(`• Transport (£${cats['Transport'].toFixed(0)}): public transit or walk.`);
  ...
  const addExclude=()=>{
    const m=newExclude.trim(); if(m&&!excludedMerchants.includes(m)) {setExcludedMerchants([...excludedMerchants,m]);setNewExclude('');}
  };
    ...
    <div className="flex flex-col min-h-screen bg-gray-50">
      ...
      <main className="flex-grow max-w-4xl mx-auto p-4 sm:p-6 space-y-6">
        ...
        <section className="bg-white p-4 rounded shadow grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <div><label>Income (£)</label><input type="number" value={income} onChange={e=>setIncome(+e.target.value)} className="border p-2 w-full"/></div>
          <div><label>Frequency</label><select value={incomeFrequency} onChange={e=>setIncomeFrequency(e.target.value)} className="border p-2 w-full"><option>weekly</option><option>monthly</option><option>yearly</option></select></div>
          <div><label>Goal Name</label><input type="text" value={goalName} onChange={e=>setGoalName(e.target.value)} className="border p-2 w-full"/></div>
          <div><label>Goal Amount (£)</label><input type="number" value={goalAmount} onChange={e=>setGoalAmount(+e.target.value)} className="border p-2 w-full"/></div>
          <div><label>Deadline</label><input type="date" value={deadline} onChange={e=>setDeadline(e.target.value)} className="border p-2 w-full"/></div>
          <div className="flex items-center mt-2"><label className="inline-flex items-center"><input type="checkbox" checked={showSuggestions} onChange={e=>setShowSuggestions(e.target.checked)} className="form-checkbox"/><span className="ml-2">Auto-suggest cancellations</span></label></div>
        </section>
        ...
        <section className="bg-white p-4 rounded shadow text-center">
          <h2 className="text-lg font-semibold mb-2">🎯 {goalName || 'Main Goal'}</h2>
          <div className="relative mx-auto w-32 h-32">
            <svg viewBox="0 0 36 36" className="transform -rotate-90 w-full h-full">
              ...
              <circle cx="18" cy="18" r="15.9155" stroke="#2196F3" strokeWidth="4" strokeDasharray={`${pct('Total')},100`} fill="none" />
            </svg>
        ...
        {/* History */}
        <section className="bg-white p-4 rounded shadow">
          <h2 className="text-lg font-semibold mb-2">History</n
