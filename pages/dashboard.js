...
export default function Dashboard() {
  ...
  const addExclude = () => {
    ...
    if (m && !excludedMerchants.includes(m)) setExcludedMerchants((p) => [...p, m]);
    setNewExclude("");
  };
  ...
  function analyze(data) {
    ...
    data.forEach(({ Description = "", Amount = 0 }) => {
      ...
      if (excludedMerchants.some((ex) => desc.includes(ex))) return;
    ...
    setTotalSpend(spend);
    setHistory((p) => [...p, { date: new Date().toISOString(), spend }]);
    const recList = Object.entries(count)
      .filter(([, c]) => c > 1 || subscriptionKeywords.some((k) => !count[k] && k.includes(k)))
      .map(([d]) => d);
    ...
    const recs = [];
    if (cats['Subscriptions']) recs.push(`• Subscriptions: cancel unused to save £20–£40.`);
    ...
    if (cats['Groceries']) recs.push(`• Try cheaper: ${groceryHints[0]||'own-brand'}`);
    recs.push('• Reallocate savings: high-yield savings, ETFs, ISA.');
      ...
      <main className="flex-grow max-w-4xl mx-auto p-6 space-y-6">
        ...
        <section className="bg-white p-4 rounded shadow text-center space-y-2">
          ...
          <div className="relative mx-auto w-32 h-32">
            <svg viewBox="0 0 36 36" className="-rotate-90 w-full h-full"><circle cx="18" cy="18" r="15.9155" stroke="#eee" strokeWidth="4" fill="none"/><circle cx="18" cy="18" r="15.9155" stroke="#2196F3" strokeWidth="4" strokeDasharray={`${goalAmount?((totalSpend/goalAmount)*100).toFixed(1):0},100`} fill="none"/></svg>
        ...
        <section className="bg-white p-4 rounded shadow">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {['Groceries','Transport','Subscriptions','Housing','Other'].map(cat=>{
              const p=pct(cat);
              return (<div key={cat} className="text-center"><p className="font-medium text-sm">{cat}</p><div className="relative mx-auto w-20 h-20"><svg viewBox="0 0 36 36" className="-rotate-90 w-full h-full"><circle cx="18" cy="18" r="15.9155" stroke="#eee" strokeWidth="4" fill="none"/><circle cx="18" cy="18" r="15.9155" stroke="#4CAF50" strokeWidth="4" strokeDasharray={`${p},100`} fill="none"/></svg><div className="absolute inset-0 flex items-center justify-center text-xs font-semibold">{p}%</div></div><p className="text-xs">£{(categorized[cat]||0).toFixed(2)}</p></div>);
            })}
          </div>
        </section>
        {/* Overview & Trend */}
